...
(defui header-section []
  ($ :header
    ...
    ($ :h1
      {:class "text-3xl mb-2 text-gray-900 dark:text-white"}
      "Simon Wasle")
    ($ :p
      {:class "text-md text-gray-900 dark:text-gray-100"}
      "developer • entrepreneur • maker")))

(defui landscape-graphic [{:keys [on-theme-toggle dark-mode?]}]
  ($ :div
...
(defui who-section []
  ($ :section
    {:class "mb-10 md:mb-16 max-w-prose mx-auto"}
    ($ :h2
      {:class "text-2xl mb-6 text-gray-900 dark:text-white"}
      "Who I am")
    ($ :div
  ...
  (let [[dark-mode? set-dark-mode!] (use-state false)
        ...
        toggle-theme (fn []
                       (let [new-dark-mode (not dark-mode?)]
                         (set-dark-mode! new-dark-mode)
                         #?(:cljs
                            (do
                              ;; Update localStorage
                              (.setItem js/localStorage "theme" (if new-dark-mode "dark" "light"))
    ...
    ($ :div
       {:class "bg-white dark:bg-gray-900"}
         ($ :div
          {:class "min-h-screen max-w-4xl mx-auto px-6 py-12"}
          ($ header-section)
          ($ landscape-graphic {:on-theme-toggle toggle-theme :dark-mode? (and hydrated? dark-mode?)})
          ($ who-section)
